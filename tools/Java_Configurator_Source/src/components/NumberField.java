...
public abstract class NumberField<T extends Comparable<T>> extends JFormattedTextField {
  ...
  public NumberField(Class<?> valueClass, T min, T max) {
    ...
    // necessary to allow invalid intermediate values (will be committed or reverted on focus out)
  ...
  public void setMinMax(T min, T max) {
    ...
    if (oldValue != null) {
      if (oldValue.compareTo(min) < 0) this.setValue(min);
      else if (oldValue.compareTo(max) > 0) this.setValue(max);
    }
  }
